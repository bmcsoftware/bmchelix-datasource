...
import { ElasticsearchQuery, LogAggregation } from './logTypes';
import {
  DataFrame,
  DataQueryResponse,
  FieldType,
...
export class LogResponse {
  ...
  processAggregationDocs(esAgg: any, aggDef: LogAggregation, target: any, table: any, props: any) {
    // add columns
  ...
  processBuckets(aggs: any, target: any, seriesList: any, table: any, props: any, depth: any) {
    ...
    for (aggId in aggs) {
      aggDef = _.find(target.sourceQuery.bucketAggs, { id: this.logToGrafanaIdFormat(aggId) });
      esAgg = aggs[aggId];
  ...
  logToGrafanaIdFormat(str: string) {
  ...
  getErrorFromLogResponse(response: any, err: any) {
    const result: any = {};
    result.data = JSON.stringify(err, null, 4);
    if (err.root_cause && err.root_cause.length > 0 && err.root_cause[0].reason) {
      result.message = err.root_cause[0].reason;
    } else {
      result.message = err.reason || 'Unkown log error response';
    }
  ...
  getTimeSeries() {
    if (this.targets.some((target: any) => target.sourceQuery.metrics.some((metric: any) => metric.type === 'raw_data' || metric.type === 'raw_document'))) {
