...
export class LogMetricAggCtrl {
  /** @ngInject */
  constructor($scope: any, uiSegmentSrv: any, $rootScope: any) {
    const metricAggs: LogAggregation[] = $scope.target.metrics;
    $scope.metricAggTypes = queryDef.getMetricAggTypes();
    ...
    $scope.onTypeChange = () => {
      ...
      if ($scope.target.bucketAggs.length === 0 &&
...
export function logMetricAgg() {
  return {
    templateUrl: TEMPLATE_BASE_URL + '/partials/log/metric_agg.html',
    controller: LogMetricAggCtrl,
    restrict: 'E',
    scope: {
      target: '=',
      index: '=',
...
coreModule.directive('logMetricAgg', logMetricAgg);
