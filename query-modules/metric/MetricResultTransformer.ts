...
export class MetricResultTransformer {
  ...
  transformMetricData(metricData: any, options: any, start: number, end: number) {
    ...
    for (const value of metricData.values) {
      let dpValue: any = parseFloat(value[1]);
      if (_.isNaN(dpValue)) {
        dpValue = null;
      }
  ...
  transformToHistogramOverTime(seriesList: TimeSeries[]) {
    ...
    */
    for (let i = seriesList.length - 1; i > 0; i--) {
      const topSeries: any = seriesList[i].datapoints;
      const bottomSeries: any = seriesList[i - 1].datapoints;
