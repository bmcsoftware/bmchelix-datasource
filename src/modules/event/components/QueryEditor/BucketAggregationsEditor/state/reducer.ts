import { defaultBucketAgg } from 'query-modules/event/event_query_def';
import { ElasticsearchQuery } from 'query-modules/event/eventTypes';
import { metricAggregationConfig } from '../../MetricAggregationsEditor/utils';
import { BucketAggregation, Terms, DateHistogram } from '../aggregations';
import { initQuery } from '../../state';
import { bucketAggregationConfig } from '../utils';
import { removeEmpty } from '../../../../utilities/utils';
  ...
  (state: ElasticsearchQuery['bucketAggs'], action: Action): ElasticsearchQuery['bucketAggs'] => {
    ...
    if (changeMetricType.match(action)) {
      ...
      if (metricAggregationConfig[action.payload.type].isSingleMetric) {
        ...
        return [{ ...(defaultBucketAgg('2') as DateHistogram), field: defaultTimeField }];
      }
      return state;
    }
