...
const getTypeOptions = (
  ...
  return (
    Object.entries(metricAggregationConfig)
      ...
      .filter(([key]) => includePipelineAggregations || !metricAggregationConfig[key]?.config?.isPipelineAgg)
      // Filtering out X-Pack plugins if X-Pack is disabled
      .filter(([key]) => (metricAggregationConfig[key]?.config?.xpack ? xpack : true))
      .map(([key]) => ({
        label: metricAggregationConfig[key].label,
