...
import { extendedStats } from '../../../../../../query-modules/event/event_query_def';
      ...
      {metric.type === 'extended_stats' && (
        <>
          {extendedStats.map((stat) => (
            <ExtendedStatSetting
              key={stat.value}
              stat={stat as ExtendedStat}
              onChange={(newValue) => dispatch(changeMetricMeta({ metric, meta: stat.value, newValue }))}
              value={
                (metric as any).meta?.[stat.value] !== undefined
                  ? !!(metric as any).meta?.[stat.value]
