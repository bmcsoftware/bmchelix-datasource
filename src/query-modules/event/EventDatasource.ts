...
export class EventDatasource extends DataSourceApi<EventDataSourceQuery, BMCDataSourceOptions> {
  ...
  refreshSettings(instSet: DataSourceInstanceSettings<BMCDataSourceOptions>) {
    ...
    this.queryBuilder = new EventQueryBuilder({
  ...
  public static getInstance(
    ...
    if (!EventDatasource.instance) {
      EventDatasource.instance = new EventDatasource(instSet, templateSrv, timeSrv);
    }
    EventDatasource.instance.refreshSettings(instSet);
  ...
  async annotationQuery(options: any): Promise<any> {
    ...
    const timeField = annotation.timeField || 'creation_time';
