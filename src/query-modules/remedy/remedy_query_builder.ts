...
export class RemedyQueryBuilder {
  ...
  build(target: any, adhocFilters?: any) {
  ...
  buildColumnSql(inputList: SelectionList[], use_distinct: boolean): string {
    let sql = EMPTY;
    sql += SQL_SELECT;
    if (use_distinct) {
        ...
        if (
          ...
          if (
            ...
            if (fieldValue.indexOf(COMMA) > 0) {
              fieldValue = fieldValue.split(COMMA).join(SINGLE_QUOTE + COMMA + SINGLE_QUOTE);
            }
